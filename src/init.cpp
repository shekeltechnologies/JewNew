...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the JEW and zJEW money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 5500, 5501));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, shekel, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Shekel specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:5500"));
...
//    strUsage += "  -anonymizeshekelamount=<n>     " + strprintf(_("Keep N JEW anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The Shekel Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that Shekel is running in a usable environment with all
...
#endif
    // Make sure only a single Shekel process is using the data directory.
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                // Shekel: load previous sessions sporks if we have them.
                ...
                // Shekel: recalculate Accumulator Checkpoints that failed to database properly
...
//    if (nAnonymizeShekelAmount < 2) nAnonymizeShekelAmount = 2;
    ...
    LogPrintf("Anonymize Shekel Amount %d\n", nAnonymizeShekelAmount);
